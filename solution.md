...
高并发 AI Agent 智能客服系统，设计目标 10000 QPS，支持退换货政策查询、订单状态查询、订单退货、仅退款、投诉等场景。
...
**问题**：单 Key RPM 上限 ≈200，10000 QPS 需要控制并发而非加连接数。
...
- **Nginx**：worker_connections 4096 × 4 = 16384 连接上限，rate=10000r/s 限流，超出 503
...
4. **高并发优化**：DeepSeek Gateway（KeyPool+排队+背压）、Nginx 10000r/s、StorageRouter、规则引擎
...
Nginx：`/api/` 全局 10000r/s + `/api/v1/auth/` 单独 5r/m（防暴力破解），`proxy_buffering off` 保证 SSE 逐帧推送。
