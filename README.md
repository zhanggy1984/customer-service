...
| 场景 | 触发示例 | 系统行为 |
|------|---------|---------|
| 退货 | 「我要退货 ORD-20240801-001」 | 校验订单归属 → 判定退货资格 → 收集原因 → 用户确认 → 创建退货单，商品状态同步流转；支持「只退手机壳」式部分退货（整件粒度，**任意轮次可补充指定**，LLM 漏提取时有规则兜底，指定商品不可退时明确提示） |
| 仅退款 | 「我想仅退款 ORD-20240806-003」 | 三级资格判定：未发货可退 / 已发货需先拒收 / 已签收必须走退货；退款金额自动过滤定制类不可退商品 |
...
推荐测试链路：**「我要退货 ORD-20240801-001」** → 回答原因 → 点确认 → 收到退货单号；再开新会话体验**多轮部分退货**：**「我要退货」→ 追问订单号 →「ORD-20240801-001」→「只退手机壳」→ 回答原因 → 确认页只见「手机壳×1，¥29.9」** → 中途切走再「继续退货」体验快照恢复 → 切 admin 体验知识库管理。
...
- **部分退货多轮指定**：任意推进轮用户都可补充「只退某商品」，items 槽合并进 `return_items`；资格已算过则回到 `check_eligibility` 携带新子集重算，避免按全量确认。LLM 漏提取 items 时，以「只退/就退/只要退/仅退/单退 + 商品名」句式规则兜底提取（含量词/语气词/多商品/否定句式处理），规避偶发漏提取导致的退错全量风险。

## 2.3 业务状态机（LangGraph）

三个业务流，每个节点可被 `取消 / 返回 / 更换 / 转人工` 全局打断：

- **退货**：收集订单 → 验证归属 → 资格判定(reasoner) → 收集原因 → 确认 → 执行 → 通知（支持部分退货：任意轮次指定商品子集，资格重算后确认页展示子集及其金额）
