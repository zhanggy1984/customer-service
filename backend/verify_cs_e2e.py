...
def _summary(events: list[dict]) -> str:
    ...
    for ev in events:
        ...
        t = ev["type"]
        if t == "answer":
            parts.append(f"answer[{len(d.get('delta',''))}字]")
        elif t == "tool_call":
...
def _check(events: list[dict], name: str, required: dict) -> None:
    ...
    for t, fields in required.items():
        ...
        # usage/answer 允许多帧，检查首帧即可；必选字段缺则报
        if bad:
...
async def _scene(token: str, sid: str, name: str, content: str, expect: str) -> list[dict]:
    ...
    # done 在最后；answer 拼接应与 done.content 一致
    answers = "".join(e["data"]["delta"] for e in events if e["type"] == "answer")
    ...
    if done is not None and answers:
        ...
        if ok:
            print("  ✅ answer 拼接 == done.content")
        else:
            print(f"  ❌ answer 拼接≠done.content: 拼={answers[:30]!r} done={done_content[:30]!r}")
    elif done is not None and not answers:
        print("  ⚠️ 无 answer 事件（规则/静态话术路径？done.content 直接提供）")
    return events
...
async def main() -> None:
    ...
    # 场景 3：政策查询（期望 tool_call + 流式 answer）
