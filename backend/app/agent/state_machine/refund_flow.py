...
class RefundState(TypedDict, total=False):
    ...
    awaiting: str
    final: bool
    tool_calls: list  # 契约透出：query_order/create_refund 动作（orchestrator 取出 emit tool_call）
...
async def _verify_order(state):
    ...
    return {
        ...
        "stage": "check_refund_eligibility",
...
async def _execute(state):
    ...
    return {
        ...
        "tool_calls": [{
            "name": "create_refund",
            "args": {"order_id": order.order_id, "reason": state.get("reason", "")},
            "result": {
                "success": result.success,
                "refund_id": result.refund_id,
                "amount": result.amount,
